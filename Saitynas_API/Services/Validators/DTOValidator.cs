...
public class DTOValidator
{
    ...
    {
        if (string.IsNullOrEmpty(parameter)) throw new DTOValidationException(ApiErrorSlug.EmptyParameter, name);
    ...
    protected static void ValidateStringLength(string parameter, string name)
    {
        ...
        if (parameter.Length > MaxStringLength) throw new DTOValidationException(ApiErrorSlug.StringTooLong, name);
    }

    protected static void ValidateIntegerIsPositive(int number, string name)
    {
        if (number <= 0) throw new DTOValidationException(ApiErrorSlug.InvalidNumber, name);
    }
